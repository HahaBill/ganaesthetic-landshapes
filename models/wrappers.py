...
class BaseModel(AbstractBaseClass, torch.nn.Module):
    ...
    def sample_np(self, z=None, n_samples=1, seed=None):
        if z is None:
            ...
            z = [torch.tensor(l).to(self.device)
                 if not torch.is_tensor(l) else l for l in z]
...
class StyleGAN2(BaseModel):
    ...
    def load_model(self):
        ...
        checkpoint = Path(
            checkpoint_root) / f'stylegan2/stylegan2_{self.outclass}_{self.resolution}.pt'

        self.model = stylegan2.Generator(
            self.resolution, 512, 8).to(self.device)
    ...
    def partial_forward(self, x, layer_name):
        ...
        if len(styles) == 1:
            ...
            latent2 = styles[1].unsqueeze(1).repeat(
                1, self.model.n_latent - inject_index, 1)
    ...
    def set_noise_seed(self, seed):
        ...
        for i in range(3, self.model.log_size + 1):
            for _ in range(2):
                self.noise.append(torch.randn(
                    1, 1, 2 ** i, 2 ** i, device=self.device))
...
class StyleGAN(BaseModel):
    ...
    def set_output_class(self, new_class):
        if self.outclass != new_class:
            raise RuntimeError(
                'StyleGAN: cannot change output class without reloading')
...
class ProGAN(BaseModel):
    ...
    def load_model(self):
        ...
        self.model = proggan.from_pth_file(
            str(checkpoint.resolve())).to(self.device)

    def sample_latent(self, n_samples=1, seed=None, truncation=None):
        if seed is None:
            # use (reproducible) global rand state
            seed = np.random.randint(np.iinfo(np.int32).max)
        noise = zdataset.z_sample_for_model(
            self.model, n_samples, seed=seed)[...]
    ...
    def partial_forward(self, x, layer_name):
        assert isinstance(
            self.model, torch.nn.Sequential), 'Expected sequential model'
...
class BigGAN(BaseModel):
    ...
    def sample_latent(self, n_samples=1, truncation=None, seed=None):
        ...
        noise_vector = biggan.truncated_noise_sample(
            truncation=truncation or self.truncation, batch_size=n_samples, seed=seed)
    ...
    def set_output_class(self, class_id):
        if isinstance(class_id, int):
            self.v_class = torch.from_numpy(
                biggan.one_hot_from_int([class_id])).to(self.device)
            self.outclass = f'class{class_id}'
        elif isinstance(class_id, str):
            self.outclass = class_id.replace(' ', '_')
            self.v_class = torch.from_numpy(
                biggan.one_hot_from_names([class_id])).to(self.device)
        else:
            raise RuntimeError(f'Unknown class identifier {class_id}')
    ...
    def partial_forward(self, x, layer_name):
        ...
        assert len(
            x) == self.model.n_latents, f'Expected {self.model.n_latents} latents, got {len(x)}'
        assert len(
...
def get_model(name, output_class, device, **kwargs):
    ...
    if name == 'DCGAN':
        ...
        warnings.filterwarnings(
            "ignore", message="nn.functional.tanh is deprecated")
